...
namespace strange.extensions.command.impl
{
    public class CommandBinder : Binder, ICommandBinder, IPooledCommandBinder, ITriggerable
    {
        ...
        private readonly HashSet<ICommand> activeCommands = new HashSet<ICommand>();
...
#if STRANGE_ANALYSIS
            strange.extensions.analysis.impl.StrangeDebugger.CommandExecute(command);
            strange.extensions.analysis.impl.StrangeDebugger.Measure(() => command.Execute(), null, command, "CommandExecutionInfo");
#else
            command.Execute();
#endif
        }
        ...
        private Pool makePoolFromType(Type type)
        {
            var poolType = typeof(Pool<>).MakeGenericType(type);

            injectionBinder.Bind(type).To(type);
            injectionBinder.Bind<Pool>().To(poolType).ToName(CommandKeys.CommandPool);
            var pool = injectionBinder.GetInstance<Pool>(CommandKeys.CommandPool);
            injectionBinder.Unbind<Pool>(CommandKeys.CommandPool);
            return pool;
        }
    }
}