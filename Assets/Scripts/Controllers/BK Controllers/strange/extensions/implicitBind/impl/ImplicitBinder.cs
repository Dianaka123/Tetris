        ...
        public virtual void ScanForAnnotatedClasses(params string[] usingNamespaces)
        {
            if (Assembly != null)
            {
                ...
                for (var ns = 0; ns < namespacesLength; ns++)
                    typesInNamespaces.AddRange(types.Where(t =>
                        !string.IsNullOrEmpty(t.Namespace) && t.Namespace.StartsWith(usingNamespaces[ns])));
                ...
                foreach (var type in typesInNamespaces)
                {
                    ...
                    if (implementedBy.Any())
                    {
                        ...
                        if (implBy.DefaultType.GetInterfaces().Contains(type)
                        ) //Verify this DefaultType exists and implements the tagged interface
                            implementedByBindings.Add(
                                new ImplicitBindingVO(type, implBy.DefaultType,
                                    implBy.Scope == InjectionBindingScope.CROSS_CONTEXT, null));
                        else
                            throw new InjectionException(
                                "Default Type: " + implBy.DefaultType.Name +
                                " does not implement annotated interface " + type.Name,
                    ...
                    if (mediated.Any())
                    {
                        ...
                        if (mediatorType == null)
                            throw new MediationException(
                                "Cannot implicitly bind view of type: " + type.Name + " due to null MediatorType",
                    ...
                    else if (mediates.Any())
                    {
                        ...
                        if (viewType == null)
                            throw new MediationException(
                                "Cannot implicitly bind Mediator of type: " + type.Name + " due to null ViewType",
                                MediationExceptionType.MEDIATOR_VIEW_STACK_OVERFLOW);
                    }
            ...
            {
                throw new InjectionException("Assembly was not initialized yet for Implicit Bindings!",
                    InjectionExceptionType.UNINITIALIZED_ASSEMBLY);
            }
        }
