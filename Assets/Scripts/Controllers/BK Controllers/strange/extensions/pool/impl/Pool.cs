...
namespace strange.extensions.pool.impl
{
    ...
    public class Pool : IPool, IPoolable
    {
        ...
        private readonly Stack instancesAvailable = new Stack();

        /// A HashSet of the objects checked out of the Pool.
        private HashSet<object> instancesInUse = new HashSet<object>();
        ...
        {
            failIf(value.GetType() != poolType,
                "Attempt to remove a instance from a pool that is of the wrong Type:\n\t\tPool type: " + poolType +
                "\n\t\tInstance type: " + value.GetType(), PoolExceptionType.TYPE_MISMATCH);
        ...
        private void failIf(bool condition, string message, PoolExceptionType type)
        {
            if (condition) throw new PoolException(message, type);
        }
        ...
        {
            failIf(value.GetType() != poolType,
                "Pool Type mismatch. Pools must consist of a common concrete type.\n\t\tPool type: " + poolType +
        ...
        protected virtual Enum constraint { get; set; }
        ...
        public virtual object GetInstance()
        {
            ...
            if (instancesToCreate > 0)
            {
                failIf(instanceProvider == null, "A Pool of type: " + poolType + " has no instance provider.",
                    PoolExceptionType.NO_INSTANCE_PROVIDER);
        ...
        public virtual void ReturnInstance(object value)
        {
            if (instancesInUse.Contains(value))
            {
                if (value is IPoolable) (value as IPoolable).Restore();
                instancesInUse.Remove(value);
                instancesAvailable.Push(value);
            }
        }
